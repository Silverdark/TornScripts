...
(async function() {
    ...
    async function initFlightClubData() {
        ...
        if (!data || currentTimestamp > data.lastUpdate + flightClubCacheTimeInSeconds * 1000) {
            const cookie = await getFlightClubCookie();
            const itemDataByItemName = await getCurrentItemDataByItemName(cookie);
    ...
    async function getFlightClubCookie() {
        const loginPageResponse = await gmFetch(`${travelWebsiteUrl}`, 'GET');
        const { responseText, responseHeaders } = loginPageResponse;

        const cookies = {
            'XSRF-TOKEN': getCookieFromResponse(responseHeaders, 'XSRF-TOKEN'),
            wtf_stuffed_flowers_inc_session: getCookieFromResponse(responseHeaders, 'wtf_stuffed_flowers_inc_session'),
        };

        if (responseText.includes('You are logged in as')) return getCookieFromResponse(loginPageResponse.responseHeaders, 'wtf_stuffed_flowers_inc_session');
        ...
        return getCookieFromResponse(loginPageResponse.responseHeaders, 'wtf_stuffed_flowers_inc_session');
    }

    async function getCurrentItemDataByItemName(cookie) {
        const plushieDataByItemName = await getItemDataByItemNameFromPage(`${travelWebsiteUrl}dashboard`, cookie);
        const flowerDataByItemName = await getItemDataByItemNameFromPage(`${travelWebsiteUrl}dashboard/flowers`, cookie);
        return Object.assign({}, plushieDataByItemName, flowerDataByItemName);
    }

    async function getItemDataByItemNameFromPage(page, cookie) {
        const response = await gmFetch(page, 'GET',
            {
                Cookie: `wtf_stuffed_flowers_inc_session=${cookie}`,
            });
    ...
    function getCookieFromResponse(headers, name) {
        const match = headers.match(new RegExp(`set-cookie:\\s*${name}=([^;]+)`, 'i'));
